...
outputDir = os.path.join(fldDatadir, 'output')
...
# imageNamesFilepath = os.path.join(fldDatadir, 'image_names.txt')
...
#     imageNamesHelen = [x for x in imageNames if 'helen' in x]
# else:
...
imagePaths = glob.glob(os.path.join(helenTrainDir, '*.jpg')) + \
             glob.glob(os.path.join(helenTestDir, '*.jpg'))

imageNames = [os.path.splitext(x)[0] for x in imagePaths]
...
for k, imageName in enumerate(imageNamesSampled):
  ...
  pointsPath = imageName + '_bv' + str(numPoints) + '.txt'
  parts = []
